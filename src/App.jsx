...
const FeatureCard = ({ icon: Icon, title, description, onClick }) => (
  <div onClick={onClick} className="cursor-pointer bg-white rounded-2xl p-6 shadow-lg min-w-[250px] flex flex-col items-center text-center transform transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl">
    {Icon && <Icon className="text-orange-500 w-8 h-8 mb-3" />}
...
export default function App() {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="flex justify-between items-center p-6 bg-black text-white">
        ...
        <div className="space-x-4">
          <button onClick={() => setView("login")} className="px-4 py-2 bg-yellow-400 rounded-lg font-bold hover:bg-yellow-300 transition">Login</button>
          <button onClick={() => setView("signup")} className="px-4 py-2 border-2 border-yellow-400 rounded-lg font-bold hover:bg-yellow-50 transition">Sign Up</button>
        </div>
      </nav>
      ...
      <section className="flex flex-col items-center justify-center flex-1 bg-gradient-to-br from-yellow-50 via-orange-100 to-red-100 text-center py-20 px-6">
        ...
        <div className="flex gap-4 flex-wrap justify-center">
          <button onClick={() => setView("login")} className="bg-yellow-400 text-black px-8 py-4 rounded-full font-bold hover:scale-105 transition">Login</button>
          <button onClick={() => setView("signup")} className="bg-black text-yellow-400 px-8 py-4 rounded-full font-bold hover:scale-105 transition">Sign Up</button>
        </div>
      </section>
  ...
  const renderForm = (type) => (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6">
      ...
      <input type="password" placeholder="Password" className="mb-4 px-4 py-2 border rounded-lg w-80"/>
      <button onClick={type === "Login" ? handleLogin : handleSignup} className="bg-yellow-400 text-black px-6 py-3 rounded-lg font-bold hover:scale-105 transition">
        Submit
      </button>
      <button onClick={() => setView("landing")} className="mt-4 underline text-gray-700">Back</button>
    </div>
  );
  ...
  const renderDashboard = () => (
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="flex justify-between items-center p-6 bg-black text-white">
        <h1 className="text-2xl font-bold">BetPhoenix</h1>
        <div className="space-x-4">
          <button onClick={() => setView("landing")} className="px-4 py-2 bg-yellow-400 rounded-lg font-bold hover:bg-yellow-300 transition">Home</button>
          <button onClick={() => setView("dashboard")} className="px-4 py-2 border-2 border-yellow-400 rounded-lg font-bold hover:bg-yellow-50 transition">Dashboard</button>
        </div>
      </nav>
      ...
      <div className="container mx-auto px-6 py-12 flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            ...
            <ul>
              <li>Team A vs Team B - Odds 1.5</li>
              <li>Team C vs Team D - Odds 2.0</li>
              <li>Team E vs Team F - Odds 3.2</li>
            </ul>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            ...
            <ul>
              <li>Bet on Team A - Won</li>
              <li>Bet on Team D - Lost</li>
              <li>Bet on Team F - Won</li>
            </ul>
          </div>
  ...
  return view === "landing" ? renderLanding()
       : view === "login" ? renderForm("Login")
       : view === "signup" ? renderForm("Sign Up")
       : renderDashboard();
}